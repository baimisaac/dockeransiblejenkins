        ...
        stage('Docker Deploy') {
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: 'dev-server', keyFileVariable: 'SSH_KEY')]) {
                    ansiblePlaybook credentialsId: 'dev-server', disableHostKeyChecking: true, extras: "-e DOCKER_TAG=${DOCKER_TAG}", installation: 'ansible', inventory: 'dev.inv', playbook: 'deploy-docker.yml', sshPrivateKey: "${SSH_KEY}"
                }
            }
        }
    }
